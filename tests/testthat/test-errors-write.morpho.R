...
test_that(
	...
	{
	expect_error(
	  write.morpho(  M = canids19x29.array, filename = "seqfile.aln" )
	)
	}
)
...
test_that(
  "There is an error when c is not length 1 or length n",
  {
    ...
    expect_error(
      write.morpho(  M = canids19x29.matrix, c = c,
                     filename = "seqfile.aln" )
    )
  }
)
...
test_that(
  "There is an error if A provided by user is not
  correct",
  {
    A2 <- A*2
...
test_that(
  "There is an error when 'method' is not used if R is passed",
  {
    expect_error(
      write.morpho(  M = canids19x29.matrix, c = 0.25,
                     R = R.shrunk, filename = "seqfile.aln" )
    )
  }
)
